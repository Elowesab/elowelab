// Distance of all kinetochores to the Metaphase line
// v1 04-Oct-2020
...
// line. The spindle length can be used to normalize the distance. If the scale was not set, the length will be in pixels.
...
//Finding the kinetochores
	...
	Dialog.addChoice("Detect kinetochores automatically?", newArray("Yes", "No"));
    Dialog.show();
    Auto = Dialog.getChoice();
	if (Auto=="Yes"){
    ...
    run("Make Binary");
    run("Set Measurements...", "area mean min centroid display redirect=None decimal=3");
    run("Analyze Particles...",  "size=5-Infinity circularity=0.2-1.00 show=[Overlay Masks] ");
