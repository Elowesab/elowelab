...
// v1 04-Oct-2020
// This macro obtains the positioniing of the two spindle poles during mitosis. It will ask for user input or it will calculate the coordinates for each pole.
// The macro will create an alignment zone according to the set number of divisions.
// The signal inside the alignment zone in the "kinetochore image" will be cleared, and the reamining chormosomes (misaligned chromosomes) will be counted with the count particle function.


// Open images. The images are identified by the order they were opened. 1 - Image containing spindle poles. 2- Kinetochore signal image. 3- Chromatin signal image
...
// Get the pole coordinates
	...
	Dialog.create("Automatically detect spindle poles?");
	Dialog.addChoice("Detect spindle poles automatically?", newArray("Yes", "No"));
    Dialog.show();
